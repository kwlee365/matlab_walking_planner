function [dcm_desired, dcm_dot_desired, vrp_desired] = CDS_DCM_Planner(dt, sim_tick, preview_window, step_time, dsp_time, current_step, w, vrpRef)
...
for i = 1:1:preview_window
    ...
    if(time < step_time - dsp_time)                                      % SSP
        ...
        dcm_desired(i,:) = vrp1 + exp(w*(time - (step_time - dsp_time)))*(dcm_ini_DS2 - vrp1);
        dcm_dot_desired(i,:) =  w*exp(w*(time - (step_time - dsp_time)))*(dcm_ini_DS2 - vrp1);
