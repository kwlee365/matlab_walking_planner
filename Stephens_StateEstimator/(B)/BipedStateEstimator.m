...
zeros(length(sim_tick))
x = [com; comdot; cop; zeros(1,length(sim_tick))];
...
for i = 1:1:length(sim_tick)-1
    sys_s1  = COM_Offset_StateEstimator_S1(i, x(:,i), sys_s1)
    sys_s12 = COM_Offset_StateEstimator_S12(i, x(:,i), sys_s12)
end
...
title('cop')
figure()
plot(sim_time, 0.02*ones(length(sim_tick),1), sim_time, Xhat_s1(:,4), sim_time, Xhat_s12(:,4))
legend('measured', 'estimated (1)', 'estimated (2)')
title('com offset')